...
Param(
	...
	[Parameter(Mandatory=$false)]
    [string] $TemplateFile = 'policyStorage.json',
    [Parameter(Mandatory=$false)]
    [string] $TemplateParametersFile = 'policyStorageBackup.parameters.json',
    [Parameter(Mandatory=$false)]
...
{
    Write-Warning "Wrong subscription, change it to [$subscription]"
    break;
}
